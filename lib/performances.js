...
// 카드에 이미 장르 배지가 나와 있으니 같은 genreCode 기준으로 필터 옵션도 제공한다.
export function getAvailableGenres() {
  const seen = new Map();
  for (const p of data) {
    if (!seen.has(p.genreCode)) seen.set(p.genreCode, p.genreEn);
  }
  return [...seen.entries()].map(([code, genreEn]) => ({ code, genreEn }));
}
 ...
 * @param {string} [opts.genre] genreCode. 미지정 시 전체 장르
 */
export function filterPerformances({ startDate, endDate, noKoreanNeeded, city, genre } = {}) {
  const start = startDate ? parseInputDate(startDate) : null;
  const end = endDate ? parseInputDate(endDate) : null;

  return data.filter((p) => {
    if (city && p.city !== city) return false;
    if (genre && p.genreCode !== genre) return false;
